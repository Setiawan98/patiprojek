...
public class DetilKerjaan extends AppCompatActivity {

    TextView textNama, textAlamat, textTelepon, textDeskripsi, textEmail, refnama, textWebsite, ratingsum, ratingpeople, textKantor, textGaji, textPengalaman;
    ...
    float lat;
    float longt;
    LinearLayout listKerjaan;
    LinearLayout linearDetil, linearJam;
    List<KomentarParent> list = new ArrayList<>();
    List<DetilKerjaanBaru> Menu = new ArrayList<>();
    List<GambarKerjaanDetil> gambarList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detil_kerjaan);
        ...
        listKerjaan = findViewById(R.id.listKerjaan);
        ...
        textKantor = findViewById(R.id.mKantor);
        textGaji= findViewById(R.id.mGaji);
        textPengalaman= findViewById(R.id.mPengalaman);
        ratingstar.setMax(5);
        getSupportActionBar().setHomeAsUpIndicator(R.drawable.ic_arrow_back_black_24dp);
        title.setText("Detil Kerjaan");
        ...
        id = bundle.getString("id_kerjaan");
        ...
        btnKomen.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(userid.equals(""))
                {
                    Toast.makeText(DetilKerjaan.this,"Silahkan login terlebih dahulu untuk tambah komentar",Toast.LENGTH_SHORT).show();
                }
                else {
                    final Bundle bundle = getIntent().getExtras();
                    final String id = bundle.getString("id_kerjaan");
                    final float hasil = ratingstar.getRating();
                    isi = komentar.getText().toString();

                    API api = RetrofitClientInstance.getRetrofitInstance().create(API.class);
                    Call<postKomentar> call = api.addKomentar(id, "kerjaan", nama, email, telp, website, isi, String.valueOf(hasil), userid);
    ...
    public void addBalas(String id, String nama, String email, String telp, String website, String isi, String parentID, String userid)
    {
        ...
        API api = RetrofitClientInstance.getRetrofitInstance().create(API.class);
        Call<postKomentar> call = api.addKomentarBalasKerjaan( id, nama, email, telp, website, parentID, isi , userid);
        call.enqueue(new Callback<postKomentar>() {
            @Override
            public void onResponse(Call<postKomentar> call, Response<postKomentar> response) {
            }
    ...
    public void getDataDetail(){
        API api = RetrofitClientInstance.getRetrofitInstance().create(API.class);
        Call<DetilKerjaanModel> call = api.detailKerjaan(id);

        call.enqueue(new Callback<DetilKerjaanModel>() {
            @Override
            public void onResponse(Call<DetilKerjaanModel> call, Response<DetilKerjaanModel> response) {


                Log.w("ResponseAsu", new Gson().toJson(response.body()));
                title.setText(response.body().getJudul());
                refnama.setText(response.body().getData().getRef_kerjaan_nama());
                ...
                textKantor.setText(response.body().getData().getKantor());
                textGaji.setText(response.body().getData().getGaji());
                textPengalaman.setText(response.body().getData().getPengalaman());
/*                senin.setText(response.body().getData().getHari_1());
                ...
                else
                {
                    ...
                    {
                        stringArray[i] = gambarList.get(i).getFile_kerjaan_img();
                    }



                }
                ViewPagerAdapter adapter = new ViewPagerAdapter(DetilKerjaan.this,stringArray);
            ...
            @Override
            public void onFailure(Call<DetilKerjaanModel> call, Throwable t) {
    ...
    public void getKomentar(){
        Toast.makeText(DetilKerjaan.this.getApplicationContext(),id, Toast.LENGTH_SHORT).show();
        API api2 = RetrofitClientInstance.getRetrofitInstance().create(API.class);
        Call<KomentarLengkap> call2 = api2.getKomentarKerjaan(id);
        call2.enqueue(new Callback<KomentarLengkap>() {
            ...
            public void onResponse(Call<KomentarLengkap> call, Response<KomentarLengkap> response) {
                ...
                Toast.makeText(DetilKerjaan.this.getApplicationContext(),"Sukses", Toast.LENGTH_SHORT).show();
            }
    ...
    public void addViewKomentar(final String id, final String nama, final String waktu, final String telp, final String email, final String website, String isi, final String pID, Float hasil){
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                listKerjaan.addView(adapter,0);
                listKerjaan.invalidate();
            }
        },2000);
