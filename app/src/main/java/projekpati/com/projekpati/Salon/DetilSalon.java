...
import projekpati.com.projekpati.Model.Salon.GambarSalonDetil;
...
public class DetilSalon extends AppCompatActivity {
    ...
    List<DetilSalonBaru> Menu = new ArrayList<>();
    List<GambarSalonDetil> gambarList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detil_salon);
        ...
        listSalon = findViewById(R.id.listSalon);
        ...
        btnKomen.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(userid.equals(""))
                {
                    Toast.makeText(DetilSalon.this,"Silahkan login terlebih dahulu untuk tambah komentar",Toast.LENGTH_SHORT).show();
                }
                else
                {
                    final Bundle bundle = getIntent().getExtras();
                    final String id = bundle.getString("id_salon");
                    final float hasil = ratingstar.getRating();
                    isi = komentar.getText().toString();

                    API api = RetrofitClientInstance.getRetrofitInstance().create(API.class);
                    Call<postKomentar> call = api.addKomentar( id, "salon",nama, email, telp, website, isi, String.valueOf(hasil), userid);
    ...
    public void addBalas(String id, String nama, String email, String telp, String website, String isi, String parentID, String userid)
    {
        ...
        API api = RetrofitClientInstance.getRetrofitInstance().create(API.class);
        Call<postKomentar> call = api.addKomentarBalasSalon( id, nama, email, telp, website, parentID, isi , userid);
        call.enqueue(new Callback<postKomentar>() {
            @Override
            public void onResponse(Call<postKomentar> call, Response<postKomentar> response) {
            }
    ...
    public void getDataDetail(){
        API api = RetrofitClientInstance.getRetrofitInstance().create(API.class);
        Call<DetilSalonModel> call = api.detailSalon(id);

        call.enqueue(new Callback<DetilSalonModel>() {
            @Override
            public void onResponse(Call<DetilSalonModel> call, Response<DetilSalonModel> response) {


                Log.w("ResponseAsu", new Gson().toJson(response.body()));
                title.setText(response.body().getJudul());
                refnama.setText(response.body().getData().getRef_salon_nama());
                ...
                else
                {
                    ...
                    {
                        stringArray[i] = gambarList.get(i).getFile_salon_img();
                    }



                }
                ViewPagerAdapter adapter = new ViewPagerAdapter(DetilSalon.this,stringArray);
            ...
            @Override
            public void onFailure(Call<DetilSalonModel> call, Throwable t) {
    ...
    public void getKomentar(){
        Toast.makeText(DetilSalon.this.getApplicationContext(),id, Toast.LENGTH_SHORT).show();
        API api2 = RetrofitClientInstance.getRetrofitInstance().create(API.class);
        Call<KomentarLengkap> call2 = api2.getKomentarSalon(id);
        call2.enqueue(new Callback<KomentarLengkap>() {
            ...
            public void onResponse(Call<KomentarLengkap> call, Response<KomentarLengkap> response) {
                ...
                Toast.makeText(DetilSalon.this.getApplicationContext(),"Sukses", Toast.LENGTH_SHORT).show();
            }
    ...
    public void addViewKomentar(final String id, final String nama, final String waktu, final String telp, final String email, final String website, String isi, final String pID, Float hasil){
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                listSalon.addView(adapter,0);
                listSalon.invalidate();
            }
        },2000);
